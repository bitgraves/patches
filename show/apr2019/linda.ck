...
-1 => int SLIDER_DETUNE_ID;
12 => int SLIDER_FILTER_CREEP_ID; // TODO: remap
13 => int SLIDER_SHUDDER_FREQ_ID;
14 => int SLIDER_MONITOR_ID;
15 => int SLIDER_FILTER_GAIN_ID;
    ...
    while (true) {
        ...
        while (min.recv(msg)) {
            if (msg.data1 == 153) { // note on
                ...
                if (msg.data2 == SLIDER_GRAINFREQ_ID) {
                ...
                } else if (msg.data2 == SLIDER_FILTER_GAIN_ID) {
                    ((msg.data3$float + 1.0) / 128.0) => float amount;
                    amount => gRezFilterGain;
                    "Pads gain: " + amount => description;
                    <<< description >>>;
                    ...
                    <<< description >>>;
                } else if (msg.data2 == SLIDER_FILTER_CREEP_ID) { // TODO: remap
                    (msg.data3$float / 127.0) => float amount;
                    amount => gFilterBend;
                    SLIDER_FILTER_CREEP_ID => evtParam.paramId;
                    evtParam.broadcast();
...
class rezFilter {
    ...
    fun void setUpAndListen(int noteIndex, float vel, float detune, int isMonotonic) {
        ...
        // 0.1 => float gainCoeff;
        0.015 * gRezFilterGain => float gainCoeff;
        ...
        lp => out;
        while (evtParam => now) {
            if (evtParam.paramId == SLIDER_PAD_OFFSET_ID || evtParam.paramId == SLIDER_FILTER_CREEP_ID) {
