...
// jump between pure and pitshifted
// rapidly (but can adjust)
// non-snapped upper bound

// think about adding env to beginning of sequence
...
-1 => int PAD_SPAWN_SAMPLER_ID;
-1 => int PAD_STOP_SAMPLERS_ID;
...
LPF lpfLeft => blackhole;
100 => lpfLeft.freq;
0.98 => lpfLeft.Q;

LPF lpfRight => Dyno gLimiter => dac;
lpfLeft.freq() => lpfRight.freq;
lpfLeft.Q() => lpfRight.Q;
gLimiter.limit();
0.3 => gLimiter.thresh;

<<< "global gain is ", gLimiter.gain() * 100.0, "%" >>>;
<<< "filter hz is ", lpfLeft.freq(), "hz" >>>;
        ...
        while (min.recv(msg)) {
            if (msg.data1 == 153) { // note on
                ...
                if (msg.data2 == SLIDER_BREADTH_ID) {
                    ...
                    evtParam.broadcast();
                    "Freq " + tremorFreq + " hz" => description;
                    <<< description >>>;
                } else if (msg.data2 == SLIDER_LPF_ID) {
                    msg.data3 $ float / 127.0 => float val;
                    Math.pow(2, val * 14.27) => lpfLeft.freq;
                    lpfLeft.freq() => lpfRight.freq;
                    ...
                    updateBalance();
                    "Balance " + voiceBalance => description;
                    <<< description >>>;
                } else if (msg.data2 == SLIDER_MONITOR_ID) {
                    (msg.data3$float / 127.0) => float amount;
                    amount => gLimiter.gain;
                    <<< "Out gain", amount >>>;
...
class Tremor {
        ...
        while (_tremorPeriod => now) {
            if (sequenceLength == 0) {
                ...
                if (stepIndex >= sequenceLength) {
                    ...
                    renderToOsc(0, _flashIntensity, 40);
                }
            }
        }
    }
    ...
    fun void _updateTremorPeriod() {
        ...
        if (tremorFreq < 30) {
            ((tremorFreq - 10.0) * 5.0) $ int => _flashIntensity;
        } else {
            100 => _flashIntensity;
        }
    }
